...
# ╔═╡ c61aad90-1673-11ec-25aa-f3560d2e906f
begin
	using DSP
	using Plots
...
# ╔═╡ d4cd0a06-a5ab-4604-be43-1b6488a484bd
md" ## 01)

Considere um sinal de audio com frequ^^encias de 0 a 4kHz, amostrado a 10hKz

Projete um sistema para aumentar a taxa dpara 15kHz. As distorçoes no sinal devem ser menores que 0,001 na amplitude, e as imagens do interpolador devem ter atenuação de ao menos 80 dB"

# ╔═╡ db35fbca-d721-40f2-8f87-e84280ca8b2f
begin
	fa = 10_000
...
# ╔═╡ 7203b3b8-056a-4c9c-b0ef-dd5a30553e62
begin
	t = 0:1/fa:3
	x = cos.(400*2π*t) + cos.(3500*2π*t)
	plot(x[1:100])
end

# ╔═╡ 2f79d934-4211-4bea-bee4-a1a498879096
begin
	function kaiser_filter(δp, δr, ωp, ωr)
	...
	md"#### Função do filtro kaiser"
end

# ╔═╡ 28e86cfd-222a-4899-8756-0da644e56e13
#filtro PB
begin
	fp = 4_000 #Hz
	ωp = 2π*fp/(fa*L)
	...
	δp = 0.001
	δr = db2amp(-80)
	PB = kaiser_filter(δp, δr, ωp, ωr)
	
	#ωr = min(π/L,π/M)
	#fr = ωr*fa*L/2/π
	#responsetype = Lowpass(fr, fs = fa*L)
	#designmethod = FIRWindow(hanning(100))
	#PB = digitalfilter(responsetype,designmethod)
	
	PBf = PolynomialRatio(PB,[1])
	ω = range(0,π, length=500)
	H =freqz(PBf, ω)
	plot(ω/2π*fl/1000, abs.(H))
	plot!(title= "Resposta em frequência do filtro", legend = false, xlabel = "f(kHz)")
	#plot(PB)
end

# ╔═╡ 59061564-b061-4a28-9653-1ef8119143dd
begin
	xL = zeros(length(x)*L)
	xL[1:L:end] = x
	xl_interpolado = filt(PB,xL)
	xm = xl_interpolado[1:M:end]
	xb = xm
end
